...
def convert_to_json(people):  # Converts list of dict into json
...
def convert_to_yaml(people):  # Converts list of dict into YAML
...
if not response.ok:
    raise Exception(
        "GET failed with status code {}".format(response.status_code))
...
mime_type, _ = mimetypes.guess_type(
    attachment_path)  # image/png -> type/subtype
...
# returns tuple of status, u need to handle smtp auth error exception
